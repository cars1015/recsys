...
class CORR:
    ...
    def _get_users_and_items(self, df):
        ...
        return users, items
    def fit_ease(self, df , lambda_: float = 0.5):
        users, items = self._get_users_and_items(df)
        values = np.ones(df.shape[0])
        X = csr_matrix((values, (users, items)))
        #Xの特異値分解と復元
        # rng = np.random.default_rng()
        #_,S,V= svds(X,k=800,return_singular_vectors="vh")
        _, S, V = randomized_svd(X, n_components=800, random_state=0)
        Y = np.dot(np.diag(S), V)
        for i in range(1,5):
            _, S, V = randomized_svd(X, n_components=800, random_state=i)
            Y += np.dot(np.diag(S), V)
            print(i)
        X=Y
        #X=X/5
...
model = CORR()
model.fit_ease(df, lambda_=14000.0)
# 
# model.B = np.where(np.abs(model.B) < threshold, 0.0, model.B)
# print(np.count_nonzero(model.B))
